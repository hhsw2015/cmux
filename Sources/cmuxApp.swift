...
private struct SettingsWindowRootView: View {
    ...
    private func setContentVisibility(_ visible: Bool) {
        if shouldRenderSettingsContent != visible {
            shouldRenderSettingsContent = visible
        }
    }

    private func isObservedWindow(_ object: Any?) -> Bool {
        guard let observed = windowReference.window else { return false }
        guard let candidate = object as? NSWindow else { return false }
