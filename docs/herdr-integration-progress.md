...
| **E2c** | UI flow: "Open Herdr Workspace (localhost)" menu item that pulls `layout.snapshot`, calls the executor with a paneFactory wiring `HerdrDisplayClient` + externalIo `TerminalPanel` per leaf, and registers everything in `HerdrPanelRegistry`. End-to-end multi-pane mirror. | pending |
| **E2d** | Outbound: hook bonsplit mutations (split / divider drag / swap / close) on herdr-backed tabs into matching RPCs. Optimistic local apply + reconcile from `LayoutChanged`. | pending |
| **E2e** | Inbound: consume `LayoutChanged` events and apply diffs back into bonsplit (other clients' edits). | pending |
