...
| **E5 (explicit Kill workspace)** | Sidebar `HerdrWorkspaceRow` context menu has Open + Kill workspace…; Kill shows a destructive confirm alert and on confirm fires `workspace.close` RPC via `HerdrOneShotRPC`, then refreshes the host's workspace list. tab.close / pane-level kill UI not added (pane close already kills via `pane.close` from the existing close-handler path). | done |
