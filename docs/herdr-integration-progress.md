...
| **E2d (split / swap)** | User splits a herdr-backed pane → `pane.split` RPC + wire new pane. User swaps tabs → `pane.swap` RPC. Both have async windows; design needs care. | pending |
