...
| **E2e-3 (multi-change)** | `applyAdditions` runs iteratively — pick an added pane whose sibling is already bound, apply, repeat. Handles mixed remove + add (removes first, then iterative add) and 2+ adds whose siblings are each other (a new pane splitting another new pane in the same event). Stalls log a debug warning. Swap events still bail. | done |
| **E2e-3 (swap)** | Pane swap events that don't preserve leaf positions. Deferred until dogfood reveals it matters. | pending |
...
| **E5 (explicit Kill workspace)** | Sidebar `HerdrWorkspaceRow` context menu has Open + Kill workspace…; Kill shows a destructive confirm alert and on confirm fires `workspace.close` RPC via `HerdrOneShotRPC`, then refreshes the host's workspace list. Plus: `Herdr → Kill Current Workspace…` menu (Cmd+Opt+Shift+K) and matching command-palette entry kill the workspace under the focused cmux pane via `HerdrKillCommands.killCurrentWorkspace()` (looks up the binding through `HerdrTabRegistry`). tab.close / pane-level kill UI not added (pane close already kills via `pane.close` from the existing close-handler path). | done |
...
| **F3 (auto on registration)** | Settings → Hosts → Add (and Edit) sheet has an "Install herdr-cmux on save" toggle, default on for new remote hosts. On save, runs `HerdrRemoteInstaller.installOnHost(_:)` against the new/updated host. No in-sheet progress UI yet — install runs detached and the sheet closes immediately; trace lives in `/tmp/cmux-debug-*.log`. | done |
...
| **F4 (command palette)** | `ContentViewHerdrCommands.swift` mirrors the existing identifier-copy palette extension pattern: `appendHerdrCommandContributions(to:)` adds 3 entries (Open Herdr Workspace localhost — with `⌥⌘H` hint, Open Herdr Workspace on `<host>` per registered remote, Install herdr-cmux on First Remote Host) and `registerHerdrCommandHandlers(_:)` wires them to the same `HerdrPanelOpener` / `HerdrRemoteInstaller` actions as the menu. | done |
...
| **F7a** (wire smoke) | `tests/test_herdr_wire_protocol.py` spawns the fork daemon in a temp HOME and exercises every RPC cmux relies on (ping, workspace lifecycle, layout.snapshot, pane.split / set_split_ratio / close, pane.swap, pane.focus, events.subscribe). New `herdr-wire-protocol-tests` CI job (Ubuntu) builds the fork at a pinned SHA — installs zig 0.15.2 first because the herdr crate's vendored libghostty-vt build.rs invokes `zig build`. | done |
| **F7** (was C12) | Full E2E: fork daemon → open panel via cmux → type → assert round-trip. Needs macOS runner. | 1 d |
| **F8** (SSH reliability) | SSHStdioTransport handles unexpected ssh exits via `Process.terminationHandler`, captures stderr tail (last ~512B) so the host row tooltip shows ssh's actual error, adds `ConnectTimeout=10` + `ServerAliveInterval=15` so dead control sockets are detected within ~45s. HerdrEventPump reads `transport.status` after stream-end to surface a friendly reason instead of "eof / api socket closed". | done |
| **F9** (pane.exited UI) | HerdrPanelRegistry flags entries on `pane.exited`, posts `cmuxHerdrPaneExited`. TerminalPanel observes its own panelId, flips `@Published herdrPaneExited`. TerminalPanelView renders a "Process exited" pill in the top-right of the panel, alongside any zmx badge. Tmux semantics preserved: pane stays alive, scrollback intact. | done |
...
| **F7** (macOS E2E) | New `herdr-live-integration-tests` macOS CI job (github-hosted macos-15, dodges warp queue). Builds the herdr fork, spawns a daemon under /tmp (AF_UNIX 104-byte limit), runs `HerdrApiClientLiveTests` + `HerdrBackendLiveTests` against the daemon socket via `CMUX_HERDR_LIVE_SOCKET`. **Marked continue-on-error** until the pre-existing cmux-unit "no such module 'cmux'" build issue is resolved separately — wire-protocol smoke job remains the hard gate. | done (advisory) |
| **F15** (CI green) | Resolved the workflow-guard ghostty-checksum gap that blocked main: rebuilt the GhosttyKit xcframework for the new merge SHA `0f9451d2d3` on a github-hosted macos-15 runner, set the `GHOSTTY_RELEASE_TOKEN` secret on hhsw2015/cmux so the workflow can publish to hhsw2015/ghostty releases, added the matching sha256 to `scripts/ghosttykit-checksums.txt`, and updated the download script to try hhsw2015/ghostty first with a manaflow-ai/ghostty fallback for older SHAs. Also fixed the auxiliary-window guard that flagged `cmux.herdrEventPumpDebug` as missing a close-shortcut owner. | done |
